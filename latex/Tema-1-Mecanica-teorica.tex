...
La transformada de Legendre permite sustituir una variable por la
derivada de una función respecto de esa variable. Para una función
diferenciable \(F(v)\), se introduce la variable conjugada
\(p = \dfrac{dF}{dv}\) y se define una nueva función:
...
entendiendo que \(v\) se expresa en función de \(p\) cuando la relación
puede invertirse. Esta transformación conserva la información de \(F\),
pero la describe mediante la variable conjugada \(p\).
