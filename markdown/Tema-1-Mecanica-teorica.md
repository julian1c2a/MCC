...
Los coeficientes $A_{mnp}$ se calculan expandiendo el desplazamiento inicial $\Psi(x,y,z,0)$ en una serie triple de Fourier. Del mismo modo, los coeficientes $B_{mnp}$ se fijan a partir de la velocidad inicial $\dfrac{\partial \Psi}{\partial t}(x,y,z,0)$. La ortogonalidad de los modos permite determinar cada coeficiente por separado mediante integrales sobre el volumen de la cavidad.
...
Para un sistema continuo, por ejemplo el desplazamiento transversal de una cuerda, el estado está descrito por un campo $f(x,t)$. La densidad lagrangiana $\mathcal{L}$ asigna una expresión local a cada punto del espacio-tiempo a partir del campo y de sus derivadas:

$$
\begin{aligned}
\mathcal{L} = \mathcal{L}\left( f, \dfrac{\partial f}{\partial t}, \dfrac{\partial f}{\partial x} \right).
\end{aligned}
$$
...
$$
\begin{aligned}
S[f] = \int_{t_1}^{t_2} \int_{x_1}^{x_2} \mathcal{L}\left( f, \dfrac{\partial f}{\partial t}, \dfrac{\partial f}{\partial x} \right) dx \, dt.
\end{aligned}
$$
